...
sil.STPRNT = function ( DESCR1, DESCR2, SPEC ) {
    ...
    getf( I ).write( SPEC.specified.format( fmt ) );
    DESCR1.addr = 0;  // OK
};
...
sil.STREAD = function ( SPEC, DESCR, EOF, ERROR, SLOC ) {
    // string read
    var file = getf( DESCR.addr ), words;
    ...
    for ( var i = 0; i < words.length; i++ ) {
        mem[ SPEC.addr + SPEC.offset + i ] = words[ i ];
    }
    return jmp( SLOC );
};
...
sil.STREAM = function ( SPEC1, SPEC2, TABLE, ERROR, RUNOUT, SLOC ) {
    ...
    var
        ...
        // The current syntax table (the table itself, not its name)
        // XXX: 'tables' need to be imported from syntax file
        // XXX: What is the type of the 'TABLE' param?
        table = tables[ TABLE ],  // assuming TABLE is a string table name, which may not be true.
