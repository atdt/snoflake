...
/*globals mem, alloc, resolve, getInt, setInt, getReal, setReal, getUint, setUint */
    ...
    specified: {
        ...
        get: function () {
            var memLength = Math.ceil( ( this.offset + this.length ) / 12 ) * 6,
                memSegment = mem.slice( this.addr, this.addr + memLength + 1 );

            return Snoflake.str.decode( memSegment ).slice( this.offset, this.length );
        },
        set: function ( s ) {
            this.addr = mem.length;
            this.offset = 0;
            this.length = s.length;
            mem = mem.concat( Snoflake.str.encode( s ) );
        }
    }
} );
...
Snoflake.Descriptor = Descriptor;
Snoflake.Specifier = Specifier;
Snoflake.datum = datum;
Snoflake.defineValues = defineValues;
Snoflake.getd = getd;
Snoflake.getspc = getspc;
