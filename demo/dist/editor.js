import{a as C,b as D,c as N,d,e as _,f as m,g as u}from"./chunks/chunk-I2WGIKSS.js";var L={"Pattern matching":{source:`READ    LINE = INPUT :F(END)
...
GOLDFINCH
BLUEFISH
`},Swap:{source:`        WORD = 'SNOBOL'
        WORD LEN(3) . HEAD REM . TAIL
        OUTPUT = TAIL HEAD
END`,input:""},Fibonacci:{source:`        A = 0
        B = 1
LOOP    OUTPUT = B
        T = A + B
        A = B
        B = LT(B, 100) T :S(LOOP)
END`,input:""},"Word tally":{source:`        COUNT = TABLE()
...
TO BE
`},"Random story":{source:`* From James F. Gimpel, "Algorithms in SNOBOL4" (1976), Ch. 13.5
...
SWAP	SWAP = $SWAP_ARG1
	...
	$SWAP_ARG2 = SWAP
	SWAP =                                  :(RETURN)
SWAP_END`}}};var K="Pattern matching",h=document.querySelector("#status"),v=document.querySelector("main"),g=document.querySelector("#input"),Y=document.querySelector("#out"),O=document.querySelector("#example"),l=document.querySelector("#theme"),M=document.querySelector("#show-input"),W=document.querySelector("#run"),F=document.querySelector("#file-tabs"),Q=document.querySelector("#editor-stack"),V=document.querySelector("#add-file"),P;function i(n,t=!1){clearTimeout(P),h.textContent=n,h.classList.toggle("error",t)}var p="snoflake-theme";function G(n){n==="auto"?delete document.documentElement.dataset.theme:document.documentElement.dataset.theme=n,l.value=n,localStorage.setItem(p,n)}l.addEventListener("change",()=>G(l.value));G(localStorage.getItem(p)||"auto");var H=D.theme({"&":{backgroundColor:"var(--bg)",color:"var(--fg)"},".cm-gutters":{backgroundColor:"var(--panel)",color:"var(--muted)",border:"none"},".cm-activeLine":{backgroundColor:"var(--active-line)"},".cm-activeLineGutter":{backgroundColor:"var(--active-line)"},".cm-content":{caretColor:"var(--accent)"},".cm-cursor, .cm-dropCursor":{borderLeftColor:"var(--accent)"},"&.cm-focused .cm-selectionBackground, .cm-selectionBackground, .cm-content ::selection":{backgroundColor:"var(--selection)"}}),k=d.define([{tag:N.lineComment,color:"var(--muted)",fontStyle:"italic"},{tag:N.meta,color:"var(--muted)"},{tag:N.labelName,color:"var(--accent)",fontWeight:"600"},{tag:N.controlKeyword,color:"var(--accent)"},{tag:N.keyword,color:"var(--accent)"},{tag:N.standard(N.variableName),color:"var(--builtin)"},{tag:N.string,color:"var(--literal)"},{tag:N.special(N.string),color:"var(--literal)"},{tag:N.number,color:"var(--literal)"}]),J=new URL("./editor-worker.js",import.meta.url);function $(n){Y.textContent+=n+`
`}var a,r=!1,c=null;function f(){a=new Worker(J,{type:"module"}),a.onmessage=n=>{let t=n.data;t.type==="ready"?(c?.(),c=null):t.type==="line"?$(t.line):t.type==="done"?(r=!1,i("Ready")):(r=!1,i(`Error: ${t.message}`,!0))},a.onerror=n=>{r=!1,i(`Error: ${n.message}`,!0)}}function I(){r&&(a.terminate(),f()),r=!0,Y.textContent="",clearTimeout(P),P=setTimeout(()=>i("Running\u2026"),200),U();let n={};for(let t of S.slice(1))n[t.name]=t.state.doc.toString();a.postMessage({source:S[0].state.doc.toString(),input:g.value,files:n})}f();var q=[m,u(),H,_(k)],o=new D({parent:Q}),S=[],R=0,B=n=>C.create({doc:n,extensions:q});function U(){S[R]&&(S[R].state=o.state)}function s(){F.replaceChildren(),S.forEach((n,t)=>{let E=t===R,e=document.createElement("span"),T=document.createElement("span");if(e.className=E?"tab active":"tab",T.className="name",T.textContent=n.name,T.title=n.name,e.append(T),e.addEventListener("click",()=>{t!==R?X(t):n.fixed||x(T,n)}),!n.fixed){let A=document.createElement("button");A.className="close",A.type="button",A.title="Remove file",A.textContent="\xD7",A.addEventListener("click",b=>{b.stopPropagation(),j(t)}),e.append(A)}F.append(e)})}function X(n){U(),R=n,o.setState(S[n].state),s(),o.focus()}function x(n,t){let E=document.createElement("input");E.value=t.name,E.setAttribute("aria-label","File name"),n.replaceChildren(E),E.focus(),E.select(),E.addEventListener("click",e=>e.stopPropagation()),E.addEventListener("blur",()=>{let e=E.value.trim();e&&e!==t.name&&!S.some(T=>T.name===e)&&(t.name=e),s()}),E.addEventListener("keydown",e=>{e.key==="Escape"&&(E.value=t.name),(e.key==="Enter"||e.key==="Escape")&&E.blur()})}function Z(n,t=""){S.push({name:n,state:B(t)}),X(S.length-1)}function j(n){U(),S.splice(n,1),R=R>n?R-1:Math.min(R,S.length-1),o.setState(S[R].state),s()}V.addEventListener("click",()=>{let n=S.length,t;do t=`FILE${n}.INC`,n++;while(S.some(E=>E.name===t));Z(t)});W.addEventListener("click",I);addEventListener("keydown",n=>{(n.metaKey||n.ctrlKey)&&n.key==="Enter"&&(n.preventDefault(),I())});M.addEventListener("change",()=>{v.classList.toggle("input-hidden",!M.checked)});function w(n){S=[{name:"Program",state:B(n.source),fixed:!0},...Object.entries(n.files??{}).map(([t,E])=>({name:t,state:B(E)}))],R=0,o.setState(S[0].state),s(),g.value=n.input}for(let n of Object.keys(L))O.add(new Option(n));O.addEventListener("change",()=>{w(L[O.value]),I()});w(L[K]);c=I;
