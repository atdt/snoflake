import{a as M,b as C,c as T,d as g,e as F,f as p}from"./chunks/chunk-5K2CWJIC.js";var O=[{name:"Pattern matching",file:"PATTERN.SNO",source:`READ    LINE = INPUT :F(END)
...
SWAP	SWAP = $SWAP_ARG1
	...
	$SWAP_ARG2 = SWAP
	SWAP =                                  :(RETURN)
SWAP_END`}}];var en="POEM.SNO",b="\0new",f=document.querySelector("#status"),i=document.querySelector("main"),K=document.querySelector("#input"),I=document.querySelector("#out"),o=document.querySelector("#example"),d=document.querySelector("#theme"),Y=document.querySelector("#show-input"),En=document.querySelector("#run"),u=document.querySelector("#file-tabs"),Sn=document.querySelector("#editor-stack"),Nn=document.querySelector("#add-file"),m;function c(n,t=!1){clearTimeout(m),f.textContent=n,f.classList.toggle("error",t)}var v="snoflake-theme";function W(n){n==="auto"?delete document.documentElement.dataset.theme:document.documentElement.dataset.theme=n,d.value=n,localStorage.setItem(v,n)}d.addEventListener("change",()=>W(d.value));W(localStorage.getItem(v)||"auto");var Rn=C.theme({"&":{backgroundColor:"var(--bg)",color:"var(--fg)"},".cm-gutters":{backgroundColor:"var(--panel)",color:"var(--muted)",border:"none"},".cm-activeLine":{backgroundColor:"var(--active-line)"},".cm-activeLineGutter":{backgroundColor:"var(--active-line)"},".cm-content":{caretColor:"var(--accent)"},".cm-cursor, .cm-dropCursor":{borderLeftColor:"var(--accent)"},"&.cm-focused .cm-selectionBackground, .cm-selectionBackground, .cm-content ::selection":{backgroundColor:"var(--selection)"}}),Tn=g.define([{tag:T.lineComment,color:"var(--muted)",fontStyle:"italic"},{tag:T.meta,color:"var(--muted)"},{tag:T.labelName,color:"var(--accent)",fontWeight:"600"},{tag:T.controlKeyword,color:"var(--accent)"},{tag:T.keyword,color:"var(--accent)"},{tag:T.standard(T.variableName),color:"var(--builtin)"},{tag:T.string,color:"var(--literal)"},{tag:T.special(T.string),color:"var(--literal)"},{tag:T.number,color:"var(--literal)"}]),an=new URL("./editor-worker.js",import.meta.url);function rn(n){I.textContent+=n+`
`}var s,l=!1,_=null;function V(){s=new Worker(an,{type:"module"}),s.onmessage=n=>{let t=n.data;t.type==="ready"?(_?.(),_=null):t.type==="line"?rn(t.line):t.type==="done"?(l=!1,c("Ready")):(l=!1,c(`Error: ${t.message}`,!0))},s.onerror=n=>{l=!1,c(`Error: ${n.message}`,!0)}}function B(){l&&(s.terminate(),V()),l=!0,I.textContent="",clearTimeout(m),m=setTimeout(()=>c("Running\u2026"),200),h();let n={};S.forEach((t,e)=>{e!==R&&(n[t.name]=t.state.doc.toString())}),s.postMessage({source:S[R].state.doc.toString(),input:K.value,files:n})}V();var An=[p,Rn,F(Tn)],L=new C({parent:Sn}),S=[],R=0,P=n=>M.create({doc:n,extensions:An});function h(){S[R]&&(S[R].state=L.state)}function U(){u.replaceChildren(),S.forEach((n,t)=>{let e=t===R,E=document.createElement("span"),a=document.createElement("span");if(E.className=e?"tab active":"tab",a.className="name",a.textContent=n.name,a.title=n.name,E.append(a),E.addEventListener("click",()=>{t!==R?Q(t):on(a,n)}),S.length>1){let A=document.createElement("button");A.className="close",A.type="button",A.title="Remove file",A.textContent="\xD7",A.addEventListener("click",nn=>{nn.stopPropagation(),Dn(t)}),E.append(A)}u.append(E)})}function Q(n){h(),R=n,L.setState(S[n].state),U(),L.focus()}function on(n,t){let e=document.createElement("input");e.value=t.name,e.setAttribute("aria-label","File name"),n.replaceChildren(e),e.focus(),e.select(),e.addEventListener("click",E=>E.stopPropagation()),e.addEventListener("blur",()=>{let E=e.value.trim();E&&E!==t.name&&!S.some(a=>a.name===E)&&(t.name=E),U()}),e.addEventListener("keydown",E=>{E.key==="Escape"&&(e.value=t.name),(E.key==="Enter"||E.key==="Escape")&&e.blur()})}function Ln(n,t=""){S.push({name:n,state:P(t)}),Q(S.length-1)}function sn(n,t){let e=`${n}${t}`;for(let E=2;S.some(a=>a.name===e);E++)e=`${n}${E}${t}`;return e}var ln=`*  Edit this program, or write your own.
        OUTPUT = 'Hello, world.'
END
`;function In(){Z([{name:"MAIN.SNO",state:P(ln)}],""),u.querySelector(".tab").classList.add("flash"),L.focus()}function Dn(n){h(),S.splice(n,1),R=R>n?R-1:Math.min(R,S.length-1),L.setState(S[R].state),U()}Nn.addEventListener("click",()=>Ln(sn("FILE",".INC")));En.addEventListener("click",B);addEventListener("keydown",n=>{(n.metaKey||n.ctrlKey)&&n.key==="Enter"&&(n.preventDefault(),B())});I.addEventListener("keydown",n=>{(n.metaKey||n.ctrlKey)&&n.key==="a"&&(n.preventDefault(),getSelection().selectAllChildren(I))});Y.addEventListener("change",()=>{i.classList.toggle("input-hidden",!Y.checked),D()});var k=document.querySelector("#sources"),H=document.querySelector("#input-section"),J=I.closest("section"),x=[...i.querySelectorAll(".gutter")],$=matchMedia("(max-width: 720px)"),G=6,w=120,q=new Map([[k,1.1],[H,.55],[J,.8]]),r=new Map(q);function D(){if($.matches){i.style.gridTemplateColumns="";return}let n=!i.classList.contains("input-hidden");x[1].style.display=n?"":"none";let t=[r.get(k)+"fr",G+"px"];n&&t.push(r.get(H)+"fr",G+"px"),t.push(r.get(J)+"fr"),i.style.gridTemplateColumns=t.join(" ")}function X(n,t){let e=n[t];for(;e&&!(e.matches("section")&&getComputedStyle(e).display!=="none");)e=e[t];return e}var N=null;function On(n,t){let e=X(t,"previousElementSibling"),E=X(t,"nextElementSibling");!e||!E||(t.setPointerCapture(n.pointerId),t.classList.add("dragging"),i.classList.add("resizing"),N={gutter:t,left:e,right:E,startX:n.clientX,leftPx:e.getBoundingClientRect().width,spanPx:e.getBoundingClientRect().width+E.getBoundingClientRect().width,spanFr:r.get(e)+r.get(E)})}function cn(n){if(!N)return;let t=Math.max(w,Math.min(N.spanPx-w,N.leftPx+(n.clientX-N.startX))),e=N.spanFr*t/N.spanPx;r.set(N.left,e),r.set(N.right,N.spanFr-e),D()}function y(){N&&(N.gutter.classList.remove("dragging"),i.classList.remove("resizing"),N=null)}function Pn(){for(let[n,t]of q)r.set(n,t);D()}for(let n of x)n.addEventListener("pointerdown",t=>On(t,n)),n.addEventListener("pointermove",cn),n.addEventListener("pointerup",y),n.addEventListener("lostpointercapture",y),n.addEventListener("dblclick",Pn);$.addEventListener("change",D);D();function Z(n,t){S=n,R=0,L.setState(S[0].state),U(),K.value=t}function j(n){Z([{name:n.file,state:P(n.source)},...Object.entries(n.files).map(([t,e])=>({name:t,state:P(e)}))],n.input)}var z=new Option("LOAD\u2026","",!0,!0);z.disabled=!0;o.add(z);o.add(new Option("NEW",b));for(let n of O){let t=new Option(n.file,n.file);t.title=n.name,o.add(t)}o.addEventListener("change",()=>{let n=o.value;o.selectedIndex=0,n===b?In():j(O.find(t=>t.file===n)),B()});j(O.find(n=>n.file===en));_=B;
