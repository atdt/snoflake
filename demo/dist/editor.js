import{a as h,b as P,c as R,d as g,e as M,f as F,g as p}from"./chunks/chunk-I2WGIKSS.js";var l={"Pattern matching":{source:`READ    LINE = INPUT :F(END)
...
SWAP	SWAP = $SWAP_ARG1
	...
	$SWAP_ARG2 = SWAP
	SWAP =                                  :(RETURN)
SWAP_END`}}};var nn="Pattern matching",Y=document.querySelector("#status"),i=document.querySelector("main"),K=document.querySelector("#input"),m=document.querySelector("#out"),B=document.querySelector("#example"),U=document.querySelector("#theme"),f=document.querySelector("#show-input"),tn=document.querySelector("#run"),G=document.querySelector("#file-tabs"),En=document.querySelector("#editor-stack"),en=document.querySelector("#add-file"),C;function D(n,t=!1){clearTimeout(C),Y.textContent=n,Y.classList.toggle("error",t)}var v="snoflake-theme";function W(n){n==="auto"?delete document.documentElement.dataset.theme:document.documentElement.dataset.theme=n,U.value=n,localStorage.setItem(v,n)}U.addEventListener("change",()=>W(U.value));W(localStorage.getItem(v)||"auto");var Sn=P.theme({"&":{backgroundColor:"var(--bg)",color:"var(--fg)"},".cm-gutters":{backgroundColor:"var(--panel)",color:"var(--muted)",border:"none"},".cm-activeLine":{backgroundColor:"var(--active-line)"},".cm-activeLineGutter":{backgroundColor:"var(--active-line)"},".cm-content":{caretColor:"var(--accent)"},".cm-cursor, .cm-dropCursor":{borderLeftColor:"var(--accent)"},"&.cm-focused .cm-selectionBackground, .cm-selectionBackground, .cm-content ::selection":{backgroundColor:"var(--selection)"}}),Nn=g.define([{tag:R.lineComment,color:"var(--muted)",fontStyle:"italic"},{tag:R.meta,color:"var(--muted)"},{tag:R.labelName,color:"var(--accent)",fontWeight:"600"},{tag:R.controlKeyword,color:"var(--accent)"},{tag:R.keyword,color:"var(--accent)"},{tag:R.standard(R.variableName),color:"var(--builtin)"},{tag:R.string,color:"var(--literal)"},{tag:R.special(R.string),color:"var(--literal)"},{tag:R.number,color:"var(--literal)"}]),Rn=new URL("./editor-worker.js",import.meta.url);function Tn(n){m.textContent+=n+`
`}var o,L=!1,d=null;function Q(){o=new Worker(Rn,{type:"module"}),o.onmessage=n=>{let t=n.data;t.type==="ready"?(d?.(),d=null):t.type==="line"?Tn(t.line):t.type==="done"?(L=!1,D("Ready")):(L=!1,D(`Error: ${t.message}`,!0))},o.onerror=n=>{L=!1,D(`Error: ${n.message}`,!0)}}function O(){L&&(o.terminate(),Q()),L=!0,m.textContent="",clearTimeout(C),C=setTimeout(()=>D("Running\u2026"),200),_();let n={};for(let t of S.slice(1))n[t.name]=t.state.doc.toString();o.postMessage({source:S[0].state.doc.toString(),input:K.value,files:n})}Q();var rn=[F,p(),Sn,M(Nn)],s=new P({parent:En}),S=[],T=0,u=n=>h.create({doc:n,extensions:rn});function _(){S[T]&&(S[T].state=s.state)}function c(){G.replaceChildren(),S.forEach((n,t)=>{let E=t===T,e=document.createElement("span"),a=document.createElement("span");if(e.className=E?"tab active":"tab",a.className="name",a.textContent=n.name,a.title=n.name,e.append(a),e.addEventListener("click",()=>{t!==T?V(t):n.fixed||an(a,n)}),!n.fixed){let A=document.createElement("button");A.className="close",A.type="button",A.title="Remove file",A.textContent="\xD7",A.addEventListener("click",j=>{j.stopPropagation(),on(t)}),e.append(A)}G.append(e)})}function V(n){_(),T=n,s.setState(S[n].state),c(),s.focus()}function an(n,t){let E=document.createElement("input");E.value=t.name,E.setAttribute("aria-label","File name"),n.replaceChildren(E),E.focus(),E.select(),E.addEventListener("click",e=>e.stopPropagation()),E.addEventListener("blur",()=>{let e=E.value.trim();e&&e!==t.name&&!S.some(a=>a.name===e)&&(t.name=e),c()}),E.addEventListener("keydown",e=>{e.key==="Escape"&&(E.value=t.name),(e.key==="Enter"||e.key==="Escape")&&E.blur()})}function An(n,t=""){S.push({name:n,state:u(t)}),V(S.length-1)}function on(n){_(),S.splice(n,1),T=T>n?T-1:Math.min(T,S.length-1),s.setState(S[T].state),c()}en.addEventListener("click",()=>{let n=S.length,t;do t=`FILE${n}.INC`,n++;while(S.some(E=>E.name===t));An(t)});tn.addEventListener("click",O);addEventListener("keydown",n=>{(n.metaKey||n.ctrlKey)&&n.key==="Enter"&&(n.preventDefault(),O())});f.addEventListener("change",()=>{i.classList.toggle("input-hidden",!f.checked),I()});var k=document.querySelector("#sources"),H=document.querySelector("#input-section"),J=m.closest("section"),x=[...i.querySelectorAll(".gutter")],q=matchMedia("(max-width: 720px)"),X=6,w=120,$=new Map([[k,1.1],[H,.55],[J,.8]]),r=new Map($);function I(){if(q.matches){i.style.gridTemplateColumns="";return}let n=!i.classList.contains("input-hidden");x[1].style.display=n?"":"none";let t=[r.get(k)+"fr",X+"px"];n&&t.push(r.get(H)+"fr",X+"px"),t.push(r.get(J)+"fr"),i.style.gridTemplateColumns=t.join(" ")}function y(n,t){let E=n[t];for(;E&&!(E.matches("section")&&getComputedStyle(E).display!=="none");)E=E[t];return E}var N=null;function Ln(n,t){let E=y(t,"previousElementSibling"),e=y(t,"nextElementSibling");!E||!e||(t.setPointerCapture(n.pointerId),t.classList.add("dragging"),i.classList.add("resizing"),N={gutter:t,left:E,right:e,startX:n.clientX,leftPx:E.getBoundingClientRect().width,spanPx:E.getBoundingClientRect().width+e.getBoundingClientRect().width,spanFr:r.get(E)+r.get(e)})}function sn(n){if(!N)return;let t=Math.max(w,Math.min(N.spanPx-w,N.leftPx+(n.clientX-N.startX))),E=N.spanFr*t/N.spanPx;r.set(N.left,E),r.set(N.right,N.spanFr-E),I()}function b(){N&&(N.gutter.classList.remove("dragging"),i.classList.remove("resizing"),N=null)}function In(){for(let[n,t]of $)r.set(n,t);I()}for(let n of x)n.addEventListener("pointerdown",t=>Ln(t,n)),n.addEventListener("pointermove",sn),n.addEventListener("pointerup",b),n.addEventListener("lostpointercapture",b),n.addEventListener("dblclick",In);q.addEventListener("change",I);I();function Z(n){S=[{name:"Program",state:u(n.source),fixed:!0},...Object.entries(n.files).map(([t,E])=>({name:t,state:u(E)}))],T=0,s.setState(S[0].state),c(),K.value=n.input}for(let n of Object.keys(l))B.add(new Option(n));B.addEventListener("change",()=>{Z(l[B.value]),O()});Z(l[nn]);d=O;
