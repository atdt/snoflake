import{a as h,b as c,c as R,d as g,e as M,f as F,g as p}from"./chunks/chunk-I2WGIKSS.js";var I={"Pattern matching":{source:`READ    LINE = INPUT :F(END)
...
SWAP	SWAP = $SWAP_ARG1
	...
	$SWAP_ARG2 = SWAP
	SWAP =                                  :(RETURN)
SWAP_END`}}};var z="Pattern matching",Y=document.querySelector("#status"),a=document.querySelector("main"),K=document.querySelector("#input"),m=document.querySelector("#out"),B=document.querySelector("#example"),U=document.querySelector("#theme"),f=document.querySelector("#show-input"),nn=document.querySelector("#run"),G=document.querySelector("#file-tabs"),tn=document.querySelector("#editor-stack"),En=document.querySelector("#add-file"),C;function l(n,t=!1){clearTimeout(C),Y.textContent=n,Y.classList.toggle("error",t)}var v="snoflake-theme";function W(n){n==="auto"?delete document.documentElement.dataset.theme:document.documentElement.dataset.theme=n,U.value=n,localStorage.setItem(v,n)}U.addEventListener("change",()=>W(U.value));W(localStorage.getItem(v)||"auto");var en=c.theme({"&":{backgroundColor:"var(--bg)",color:"var(--fg)"},".cm-gutters":{backgroundColor:"var(--panel)",color:"var(--muted)",border:"none"},".cm-activeLine":{backgroundColor:"var(--active-line)"},".cm-activeLineGutter":{backgroundColor:"var(--active-line)"},".cm-content":{caretColor:"var(--accent)"},".cm-cursor, .cm-dropCursor":{borderLeftColor:"var(--accent)"},"&.cm-focused .cm-selectionBackground, .cm-selectionBackground, .cm-content ::selection":{backgroundColor:"var(--selection)"}}),Sn=g.define([{tag:R.lineComment,color:"var(--muted)",fontStyle:"italic"},{tag:R.meta,color:"var(--muted)"},{tag:R.labelName,color:"var(--accent)",fontWeight:"600"},{tag:R.controlKeyword,color:"var(--accent)"},{tag:R.keyword,color:"var(--accent)"},{tag:R.standard(R.variableName),color:"var(--builtin)"},{tag:R.string,color:"var(--literal)"},{tag:R.special(R.string),color:"var(--literal)"},{tag:R.number,color:"var(--literal)"}]),Nn=new URL("./editor-worker.js",import.meta.url);function Rn(n){m.textContent+=n+`
`}var o,L=!1,d=null;function Q(){o=new Worker(Nn,{type:"module"}),o.onmessage=n=>{let t=n.data;t.type==="ready"?(d?.(),d=null):t.type==="line"?Rn(t.line):t.type==="done"?(L=!1,l("Ready")):(L=!1,l(`Error: ${t.message}`,!0))},o.onerror=n=>{L=!1,l(`Error: ${n.message}`,!0)}}function D(){L&&(o.terminate(),Q()),L=!0,m.textContent="",clearTimeout(C),C=setTimeout(()=>l("Running\u2026"),200),_();let n={};for(let t of S.slice(1))n[t.name]=t.state.doc.toString();o.postMessage({source:S[0].state.doc.toString(),input:K.value,files:n})}Q();var Tn=[F,p(),en,M(Sn)],s=new c({parent:tn}),S=[],T=0,u=n=>h.create({doc:n,extensions:Tn});function _(){S[T]&&(S[T].state=s.state)}function O(){G.replaceChildren(),S.forEach((n,t)=>{let E=t===T,e=document.createElement("span"),r=document.createElement("span");if(e.className=E?"tab active":"tab",r.className="name",r.textContent=n.name,r.title=n.name,e.append(r),e.addEventListener("click",()=>{t!==T?V(t):n.fixed||rn(r,n)}),!n.fixed){let i=document.createElement("button");i.className="close",i.type="button",i.title="Remove file",i.textContent="\xD7",i.addEventListener("click",Z=>{Z.stopPropagation(),An(t)}),e.append(i)}G.append(e)})}function V(n){_(),T=n,s.setState(S[n].state),O(),s.focus()}function rn(n,t){let E=document.createElement("input");E.value=t.name,E.setAttribute("aria-label","File name"),n.replaceChildren(E),E.focus(),E.select(),E.addEventListener("click",e=>e.stopPropagation()),E.addEventListener("blur",()=>{let e=E.value.trim();e&&e!==t.name&&!S.some(r=>r.name===e)&&(t.name=e),O()}),E.addEventListener("keydown",e=>{e.key==="Escape"&&(E.value=t.name),(e.key==="Enter"||e.key==="Escape")&&E.blur()})}function an(n,t=""){S.push({name:n,state:u(t)}),V(S.length-1)}function An(n){_(),S.splice(n,1),T=T>n?T-1:Math.min(T,S.length-1),s.setState(S[T].state),O()}En.addEventListener("click",()=>{let n=S.length,t;do t=`FILE${n}.INC`,n++;while(S.some(E=>E.name===t));an(t)});nn.addEventListener("click",D);addEventListener("keydown",n=>{(n.metaKey||n.ctrlKey)&&n.key==="Enter"&&(n.preventDefault(),D())});f.addEventListener("change",()=>{a.classList.toggle("input-hidden",!f.checked),P()});var H=document.querySelector("#sources"),k=document.querySelector("#input-section"),J=m.closest("section"),x=[...a.querySelectorAll(".gutter")],q=matchMedia("(max-width: 720px)"),X=6,w=120,A=new Map([[H,1.1],[k,.55],[J,.8]]);function P(){if(q.matches){a.style.gridTemplateColumns="";return}let n=!a.classList.contains("input-hidden");x[1].style.display=n?"":"none";let t=[A.get(H)+"fr",X+"px"];n&&t.push(A.get(k)+"fr",X+"px"),t.push(A.get(J)+"fr"),a.style.gridTemplateColumns=t.join(" ")}function y(n,t){let E=n[t];for(;E&&!(E.matches("section")&&getComputedStyle(E).display!=="none");)E=E[t];return E}var N=null;function on(n,t){let E=y(t,"previousElementSibling"),e=y(t,"nextElementSibling");!E||!e||(t.setPointerCapture(n.pointerId),t.classList.add("dragging"),a.classList.add("resizing"),N={gutter:t,left:E,right:e,startX:n.clientX,leftPx:E.getBoundingClientRect().width,spanPx:E.getBoundingClientRect().width+e.getBoundingClientRect().width,spanFr:A.get(E)+A.get(e)})}function Ln(n){if(!N)return;let t=Math.max(w,Math.min(N.spanPx-w,N.leftPx+(n.clientX-N.startX))),E=N.spanFr*t/N.spanPx;A.set(N.left,E),A.set(N.right,N.spanFr-E),P()}function b(){N&&(N.gutter.classList.remove("dragging"),a.classList.remove("resizing"),N=null)}for(let n of x)n.addEventListener("pointerdown",t=>on(t,n)),n.addEventListener("pointermove",Ln),n.addEventListener("pointerup",b),n.addEventListener("lostpointercapture",b);q.addEventListener("change",P);P();function $(n){S=[{name:"Program",state:u(n.source),fixed:!0},...Object.entries(n.files).map(([t,E])=>({name:t,state:u(E)}))],T=0,s.setState(S[0].state),O(),K.value=n.input}for(let n of Object.keys(I))B.add(new Option(n));B.addEventListener("change",()=>{$(I[B.value]),D()});$(I[z]);d=D;
