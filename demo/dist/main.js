...
`)+`
`}function X(E,e={}){let O=ee(),n=ee(),s=e.file||"demo.sno",o=e.input||"input.txt",t=new Map([[s,E],[o,e.inputText||""]]),i=new q({...e,file:s,input:e.inputText===void 0?void 0:o,stdout:O,stderr:n,loader:{load(r){if(!t.has(r))throw new Error("No demo file named "+r);return t.get(r)}}});try{i.run(Q)}catch(r){n.write("Execution error: "+(r&&r.message||r))}return{stdout:Ee(O.lines),stderr:Ee(n.lines),exitCode:i.exitCode}}function K(E){let e=new j({doc:E.value,extensions:[J,$()]});return e.contentDOM.setAttribute("aria-label",E.getAttribute("aria-label")||""),E.replaceWith(e.dom),{getValue(){return e.state.doc.toString()},setValue(O){e.dispatch({changes:{from:0,to:e.state.doc.length,insert:O}})}}}var te=`* Split each input line into words with Intl.Segmenter,
...
\u0E2A\u0E27\u0E31\u0E2A\u0E14\u0E35\u0E0A\u0E32\u0E27\u0E42\u0E25\u0E01!
\u041F\u0440\u0438\u0432\u0435\u0442 \u043C\u0438\u0440!
`;function Oe(){let E=K(document.querySelector("#batch-io-source")),e=document.querySelector("#batch-io-input"),O=document.querySelector("#batch-io-output"),n=document.querySelector("#batch-io-status"),s=document.querySelector("#batch-io-run"),o=document.querySelector("#batch-io-reset");function t(T){n.textContent=T}function i(){O.textContent="",t("Running");try{let T=X(E.getValue(),{inputText:e.value});O.textContent=[T.stdout,T.stderr].filter(Boolean).join(`
`)||"(no output)",t(T.stderr||T.exitCode?"Error":"Finished")}catch(T){O.textContent="Execution error: "+(T&&T.message||T),t("Error")}}function r(){O.textContent="",t("Loading");try{E.setValue(te),e.value=Ye,t("Ready")}catch(T){O.textContent=T.message,t("Error")}}s.addEventListener("click",i),o.addEventListener("click",r),r()}var re=`* PLEASE WAIT A MINUTE WHILE I PREPARE TO BE YOUR CONSULTANT.\r
...
THAT'S//"\r
        :(MAIN)\r
END\r
`;function ne(){let E=K(document.querySelector("#interactive-io-source")),e=document.querySelector("#interactive-io-conversation"),O=document.querySelector("#interactive-io-status"),n=document.querySelector("#interactive-io-restart"),s=document.querySelector("#interactive-io-reset"),o=document.querySelector("#interactive-io-input-form"),t=document.querySelector("#interactive-io-input-line"),i=document.querySelector("#interactive-io-send"),r=document.querySelector("#interactive-io-eof"),T=null,a=!1;function L(S){O.textContent=S}function l(S){t.disabled=i.disabled=r.disabled=!S,o.classList.toggle("disabled",!S)}function N(S,A="program"){let u=document.createElement("div");u.className="terminal-line "+A,u.textContent=S,e.append(u),e.scrollTop=e.scrollHeight}function F(){T=null,a=!1,l(!1)}function Y(){F(),e.textContent="",L("Running"),T=_({source:E.getValue(),onOutput:S=>N(S),onError:S=>N(S,"error"),onDone:S=>{a=!1,l(!1),L(S?"Error":"Finished")}}),a=!0,l(!0),T.start()}function V(){F(),e.textContent="",L("Loading");try{E.setValue(re),L("Ready"),Y()}catch(S){N(S.message,"error"),L("Error")}}o.addEventListener("submit",function(S){if(S.preventDefault(),!a||!T)return;let A=t.value;t.value="",N("> "+A,"input"),T.send(A)}),t.addEventListener("keydown",function(S){S.key!=="Enter"||S.shiftKey||(S.preventDefault(),o.requestSubmit())}),r.addEventListener("click",function(){!a||!T||(N("<EOF>","input"),T.end(),l(!1))}),n.addEventListener("click",Y),s.addEventListener("click",V),l(!1),V()}function b(E,e){for(let[O,n]of Object.entries(e)){let s=document.createElement("option");s.value=O,s.textContent=n.label,E.append(s)}}function v(E){let e=E.getContext("2d");return e.lineWidth=1,e.lineCap="round",e.beginPath(),{"WIDTH()INTEGER":()=>E.width,"HEIGHT()INTEGER":()=>E.height,"COLOR(STRING)":O=>{e.fillStyle=e.strokeStyle=O},"CLEAR()":()=>e.fillRect(0,0,E.width,E.height),"FILLRECT(REAL,REAL,REAL,REAL)":(O,n,s,o)=>e.fillRect(O,n,s,o),"MOVETO(REAL,REAL)":(O,n)=>e.moveTo(O,n),"LINETO(REAL,REAL)":(O,n)=>e.lineTo(O,n),"STROKE()":()=>{e.stroke(),e.beginPath()}}}var z={koch:{label:"Koch snowflake",axiom:"F++F++F",rules:{F:"F-F++F-F"},depth:4,angle:60,startAngle:0},"koch-square":{label:"Square Koch curve",axiom:"F",rules:{F:"F+F-F-F+F"},depth:3,angle:90,startAngle:0},dragon:{label:"Dragon curve",axiom:"FX",rules:{X:"X+YF+",Y:"-FX-Y"},depth:11,angle:90,startAngle:0},hilbert:{label:"Hilbert curve",axiom:"A",rules:{A:"+BF-AFA-FB+",B:"-AF+BFB+FA-"},depth:5,angle:90,startAngle:0},plant:{label:"Fractal plant",axiom:"X",rules:{X:"F+[[X]-X]-F[-FX]+X",F:"FF"},depth:5,angle:25,startAngle:-90}};function oe(E,e,O){return{...v(E),"AXIOM()STRING":()=>e.axiom,"DEPTH()INTEGER":()=>e.depth,"ANGLE()REAL":()=>e.angle,"STARTANGLE()REAL":()=>e.startAngle,"LOOKUP(STRING)STRING":n=>e.rules[n]||"","EMIT(STRING)":n=>{O&&O(n)}}}var Te=`* L-system: SNOBOL rewrites the grammar, then walks the result as a
...
END
`;var Ae=160;function Re(){let E=K(document.querySelector("#lsystem-source")),e=document.querySelector("#lsystem-canvas"),O=document.querySelector("#lsystem-strings"),n=document.querySelector("#lsystem-preset"),s=document.querySelector("#lsystem-run"),o=document.querySelector("#lsystem-status");function t(a){o.textContent=a}function i(a,L){let l=document.createElement("div"),N=document.createElement("div"),F=document.createElement("span"),Y=document.createElement("div");l.className="strings-row",N.className="gen",N.textContent="gen "+a,F.className="len",F.textContent=L.length+" ch",N.append(F),Y.className="body",Y.textContent=L.length>Ae?L.slice(0,Ae)+"\u2026":L,l.append(N,Y),O.append(l)}function r(){let a=z[n.value];a&&(O.textContent="",t("Drawing"),requestAnimationFrame(function(){let L=0,l=oe(e,a,function(N){i(L++,N)});try{let N=X(E.getValue(),{extensions:l});N.stderr?(t("Error"),console.error(N.stderr)):t("Drawn")}catch(N){t("Error"),console.error(N)}}))}function T(){t("Loading");try{E.setValue(Te),b(n,z),t("Ready"),r()}catch(a){t("Error"),console.error(a)}}s.addEventListener("click",r),n.addEventListener("change",r),T()}var k={rule30:{label:"Rule 30 (chaos)",rule:30,init:"seed"},rule90:{label:"Rule 90 (Sierpinski)",rule:90,init:"seed"},rule110:{label:"Rule 110 (gliders)",rule:110,init:"random"},rule184:{label:"Rule 184 (traffic)",rule:184,init:"random"},rule54:{label:"Rule 54",rule:54,init:"seed"}};function ie(E,e){return{...v(E),"RULE()INTEGER":()=>e.rule,"INITKIND()STRING":()=>e.init,"ROWS()INTEGER":()=>80,"RANDBIT()INTEGER":()=>Math.random()<.5?1:0}}var se=`* Elementary 1D cellular automaton, end to end in SNOBOL: decode the
...
END
`;function ae(){let E=K(document.querySelector("#cellular-automata-source")),e=document.querySelector("#cellular-automata-canvas"),O=document.querySelector("#cellular-automata-preset"),n=document.querySelector("#cellular-automata-run"),s=document.querySelector("#cellular-automata-status");function o(r){s.textContent=r}function t(){let r=k[O.value];r&&(o("Running"),requestAnimationFrame(function(){let T=ie(e,r);try{let a=X(E.getValue(),{extensions:T});a.stderr?(o("Error"),console.error(a.stderr)):o("Done")}catch(a){o("Error"),console.error(a)}}))}function i(){o("Loading");try{E.setValue(se),b(O,k),o("Ready"),t()}catch(r){o("Error"),console.error(r)}}n.addEventListener("click",t),O.addEventListener("change",t),i()}var de={PRK:[122,168,116],FND:[180,173,158],BRC:[173,95,70],GLS:[134,195,220],RUF:[82,88,96]},g={PY:1,MY:.28,PX:.85,MX:.55,PZ:.42,MZ:.88},De={x:0,y:1,z:0};function Pe(E,e){return{x:E.x-e.x,y:E.y-e.y,z:E.z-e.z}}function Z(E,e){return E.x*e.x+E.y*e.y+E.z*e.z}function Ne(E,e){return{x:E.y*e.z-E.z*e.y,y:E.z*e.x-E.x*e.z,z:E.x*e.y-E.y*e.x}}function Se(E){let e=Math.hypot(E.x,E.y,E.z)||1;return{x:E.x/e,y:E.y/e,z:E.z/e}}function G(E,e){return"rgb("+Math.round(E[0]*e)+","+Math.round(E[1]*e)+","+Math.round(E[2]*e)+")"}function le(E){let e=E.getContext("2d"),O=[],n=1,s=1,o=-.6,t=.55,i=150,r=!0,T=!0;function a(){let A=globalThis.devicePixelRatio||1,u=E.getBoundingClientRect();n=Math.max(u.width,1),s=Math.max(u.height,1),E.width=Math.round(n*A),E.height=Math.round(s*A),e.setTransform(A,0,0,A,0,0),r=!0}function L(A,u,y,w,c,U,p,h){let M=Pe(A,u),f=Z(M,y),C=Z(M,w),B=Z(M,c);return B<.5?null:{sx:U*f/B+p,sy:-U*C/B+h,cz:B}}function l(A,u,y,w,c,U,p,h,M,f){let C=[],B=0;for(let m=0;m<4;m++){let R=L(u[m],y,w,c,U,p,h,M);if(!R)return;C.push(R),B+=R.cz}A.push({pts:C,color:f,depth:B})}function N(){let A=Math.cos(t),u=Math.sin(t),y=Math.cos(o),w=Math.sin(o),c={x:i*A*w,y:i*u,z:i*A*y},U=Se({x:-c.x,y:-c.y,z:-c.z}),p=Se(Ne(U,De)),h=Ne(p,U),M=Math.max(n,s)*1.05,f=n/2,C=s/2,B=e.createLinearGradient(0,0,0,s);B.addColorStop(0,"#d3dde7"),B.addColorStop(1,"#eef2ee"),e.fillStyle=B,e.fillRect(0,0,n,s);let m=[];for(let R of O){let I=R.x,W=R.y,d=R.z,D=R.x+R.w,P=R.y+R.h,H=R.z+R.d,x=de[R.color]||[200,200,200];c.x>D&&l(m,[{x:D,y:W,z:d},{x:D,y:W,z:H},{x:D,y:P,z:H},{x:D,y:P,z:d}],c,p,h,U,M,f,C,G(x,g.PX)),c.x<I&&l(m,[{x:I,y:W,z:H},{x:I,y:W,z:d},{x:I,y:P,z:d},{x:I,y:P,z:H}],c,p,h,U,M,f,C,G(x,g.MX)),c.y>P&&l(m,[{x:I,y:P,z:d},{x:D,y:P,z:d},{x:D,y:P,z:H},{x:I,y:P,z:H}],c,p,h,U,M,f,C,G(x,g.PY)),c.y<W&&l(m,[{x:I,y:W,z:H},{x:D,y:W,z:H},{x:D,y:W,z:d},{x:I,y:W,z:d}],c,p,h,U,M,f,C,G(x,g.MY)),c.z>H&&l(m,[{x:D,y:W,z:H},{x:I,y:W,z:H},{x:I,y:P,z:H},{x:D,y:P,z:H}],c,p,h,U,M,f,C,G(x,g.PZ)),c.z<d&&l(m,[{x:I,y:W,z:d},{x:D,y:W,z:d},{x:D,y:P,z:d},{x:I,y:P,z:d}],c,p,h,U,M,f,C,G(x,g.MZ))}m.sort((R,I)=>I.depth-R.depth);for(let R of m)e.fillStyle=R.color,e.strokeStyle=R.color,e.lineWidth=.6,e.beginPath(),e.moveTo(R.pts[0].sx,R.pts[0].sy),e.lineTo(R.pts[1].sx,R.pts[1].sy),e.lineTo(R.pts[2].sx,R.pts[2].sy),e.lineTo(R.pts[3].sx,R.pts[3].sy),e.closePath(),e.fill(),e.stroke()}function F(){T&&(o+=.0035,r=!0),r&&(N(),r=!1),requestAnimationFrame(F)}let Y=null,V=()=>{T=!1};E.addEventListener("pointerdown",A=>{E.setPointerCapture(A.pointerId),Y={x:A.clientX,y:A.clientY},V()}),E.addEventListener("pointermove",A=>{if(!Y)return;let u=A.clientX-Y.x,y=A.clientY-Y.y;Y.x=A.clientX,Y.y=A.clientY,o-=u*.008,t=Math.max(.05,Math.min(Math.PI/2-.05,t+y*.008)),r=!0});let S=()=>{Y=null};return E.addEventListener("pointerup",S),E.addEventListener("pointercancel",S),E.addEventListener("wheel",A=>{A.preventDefault(),i=Math.max(60,Math.min(360,i*(1+A.deltaY*.001))),r=!0,V()},{passive:!1}),globalThis.addEventListener("resize",a),a(),requestAnimationFrame(F),{addBox(A){O.push(A),r=!0},clear(){O.length=0,r=!0},count(){return O.length},resetCamera(){o=-.6,t=.55,i=150,T=!0,r=!0}}}var Le=`* Snoflake browser demo: a 3-D shape grammar.
*
...
R_BAND  ARGS ARGS7
        ...
        :(LOOP)
END
`;var Me=new URL("./shape-worker.js",import.meta.url);function ce(){let E=K(document.querySelector("#shape-grammar-source")),e=document.querySelector("#shape-grammar-canvas"),O=document.querySelector("#shape-grammar-status"),n=document.querySelector("#shape-grammar-restart"),s=document.querySelector("#shape-grammar-reset"),o=le(e),t=null;function i(L){O.textContent=L}function r(){t&&(t.terminate(),t=null)}function T(){r(),o.clear(),o.resetCamera();let L=0;t=new Worker(Me,{type:"module"}),t.addEventListener("message",function(l){let N=l.data;N.type==="box"?(o.addBox(N.box),i(++L+" boxes")):N.type==="done"?i(L+" boxes \xB7 finished"):N.type==="stderr"&&i("Error: "+N.line)}),t.addEventListener("error",function(l){i("Error: "+l.message)}),i("Running"),t.postMessage({type:"start",source:E.getValue()})}function a(){r(),o.clear(),i("Loading");try{E.setValue(Le),i("Ready"),T()}catch(L){i(L.message)}}n.addEventListener("click",T),s.addEventListener("click",a),a()}Oe();ne();Re();ae();ce();
