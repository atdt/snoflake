...
function runDeno( forwarded, output ) {
    ...
    if ( result.error ) {
        if ( result.error.code === 'ENOENT' ) {
            throw new Error(
                'deno not found on PATH; install Deno or use --runtime=node',
            );
        }
        throw result.error;
    }
...
function runNode( forwarded, output ) {
    ...
    if ( benchResult.status !== 0 ) {
        throw new Error(
            'bench-snoflake.js exited with status ' + benchResult.status,
        );
    }
    ...
    if ( created.length > 1 ) {
        console.error(
            'warning: multiple new isolate logs (%d); using the largest',
            created.length,
        );
    }
    created.sort( ( a, b ) =>
        fs.statSync( path.join( PROFILES_DIR, b ) ).size -
    ...
    if ( procResult.status !== 0 ) {
        throw new Error(
            '--prof-process exited with status ' + procResult.status + '\n' +
                procResult.stderr,
        );
    }

    const reportPath = output ||
        path.join( PROFILES_DIR, timestamp() + '.txt' );
...
function main() {
    const { forwarded, output, runtime } = splitArgs(
